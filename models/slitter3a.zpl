...
var p[C] >= 1 <= card(C);
var s[C*I0] >= 0;
var z[C*I0*I0] binary;

# Funcion objetivo
maximize fobj: sum <c,i,j> in C*I0*I0: z[c,i,j] - (card(C) - 1);
...
subto defzlt: forall <c,cp,i,ip> in C*C*I0*I0 with i <= items[c] and ip <= items[cp]:
    s[c,i] - s[cp,ip] <= ancho * (2 - x[c,cp] - z[c,i,ip]);

subto defzgt: forall <c,cp,i,ip> in C*C*I0*I0 with i <= items[c] and ip <= items[cp]:
    s[cp,ip] - s[c,i] <= ancho * (2 - x[c,cp] - z[c,i,ip]);

# Compatibilidad entre z y x
subto compatzx: forall <c,cp,i,j> in C*C*I0*I0 with j > items[cp]:
    z[c,i,j] <= 1 - x[c,cp];
    
subto lastx: forall <c,i,j> in C*I0*I0:
    z[c,i,j] <= 1 - x[c,0];
...
subto nullz: forall <c,i,j> in C*I0*I0 with i > items[c]:
    z[c,i,j] == 0;

# Refuerzo
subto ref1: forall <c,i> in C*I0:
    sum <ip> in I0: z[c,i,ip] <= 1;

subto ref2: forall <c,i> in C*I0:
    sum <ip> in I0: z[c,ip,i] <= 1;
