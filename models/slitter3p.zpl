...
# Funcion objetivo
maximize fobj: sum <c,i,cp,j> in C*I*C*I: z[c,i,cp,j];
...
subto nullz: forall <c,i,cp,j> in C*I*C*I with i > items[c]:
    z[c,i,cp,j] == 0;

# Refuerzo
subto ref1: forall <c,i> in C*I:
    sum <cp,ip> in C*I0: z[c,i,cp,ip] <= 1;

subto ref2: forall <c,i> in C*I:
    sum <cp,ip> in C*I0: z[c,ip,cp,i] <= 1;
    
subto ref3: forall <c,i,cp,j> in C*I*C*I with i <= items[c] and <c-1,cp-1,i-1,j-1> in posibles:
    z[c,i,cp,j] <= x[c,cp];

subto ref4: forall <c,i,cp,j> in C*I*C*I with i <= items[c] and not (<c-1,cp-1,i-1,j-1> in posibles):
    z[c,i,cp,j] <= 0;

subto ref5: forall <c,i,cp,j> in C*I*C*I with i <= items[c] and <c-1,cp-1,i-1,j-1> in posiblessinultimo:
    z[c,i,cp,j] <= y[c,i,0] + x[c,cp];
